...
> **视频内的命令可能有误，一切步骤以本安装教程内的为准** > **验证 ticket 时，最好使用 edge 浏览器**
...
- 提示 `qq版本过低` ？或者 `登录失败，建议升级最新版本后重试，或通过问题反馈与我们联系。` ？或者 `当前网络不稳定，登录失败。推荐使用常用设备或通过手机号登录。` ？<br>
  ...
  5. `node app` 正常启动云崽即可，目前<br>**必须使用密码登录**<br>**必须使用密码登录**<br>**必须使用密码登录**<br>可以输入以下命令来切换登录方式：
     ...
     > 如果还是登录不上的话，请换个 QQ 号(可以重新注册一个小号)**(换号之前一定要记得删除上方第四点提到的 device.json)**<br>
     > 换完号还是不行的话，那我就不知道了，不要来问我！！！
...
5. `node app` 正常启动云崽即可，目前<br>**必须使用密码登录**<br>**必须使用密码登录**<br>**必须使用密码登录**<br>可以输入以下命令来切换登录方式：
   ...
   ```
   可以修改登录方式，并且如果遇到密保验证的话，请选择**短信验证码**验证
   > 如果还是登录不上的话，请换个 QQ 号(可以重新注册一个小号)<br>
   > 换完号还是不行的话，那我就不知道了，不要来问我！！！！
